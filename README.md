<p align="center">
  <img src="./metis-ui-main/public/metis-main-graphic.png" alt="Metis Main Graphic" width="853" height="422">
</p>
...
> Metis is currently in beta! Expect things to not work 100%, and please feel free to share any feedback and contribute with fixes/new features!

Metis is the **command center** for AI coding assistants. For you, it's a sleek interface to manage knowledge, context, and tasks for your projects. For the AI coding assistant(s), it's a **Model Context Protocol (MCP) server** to collaborate on and leverage the same knowledge, context, and tasks. Connect Claude Code, Kiro, Cursor, Windsurf, etc. to give your AI agents access to:

- **Your documentation** (crawled websites, uploaded PDFs/docs)
- **Smart search capabilities** with advanced RAG strategies
- **Task management** integrated with your knowledge base
- **Real-time updates** as you add new content and collaborate with your coding assistant on tasks
- **Much more** coming soon to build Metis into an integrated environment for all context engineering

This new vision for Metis replaces the old one (the agenteer). Metis used to be the AI agent that builds other agents, and now you can use Metis to do that and more.

> It doesn't matter what you're building or if it's a new/existing codebase - Metis's knowledge and task management capabilities will improve the output of **any** AI driven coding.

## 🔗 Important Links

- **[Contributing Guide](CONTRIBUTING.md)** - How to get involved and contribute to Metis
...
1. **Clone Repository**:
   ```bash
   git clone https://github.com/uncutrush/metis.git
   ```
   ```bash
   cd metis
   ```
   
   **Note:** The `main` branch contains the latest features. For stable releases, check available tags.
...
4. **Start Services** (choose one):
   ...
   **Full Docker Mode (Recommended for Normal Metis Usage)**
...
<details>
<summary>⚠️ <strong>Reset Database - This will delete ALL data for Metis!</strong></summary>

1. **Run Reset Script**: In your Supabase SQL Editor, run the contents of `migration/RESET_DB.sql`

   ⚠️ WARNING: This will delete all Metis specific tables and data! Nothing else will be touched in your DB though.
...
| -------------------------- | -------------------------- | --------------------- | ------------------------------------------ |
| **Web Interface**          | metis-ui                  | http://localhost:3737 | Main dashboard and controls                |
| **API Service**            | metis-server              | http://localhost:8181 | Web crawling, document processing          |
| **MCP Server**             | metis-mcp                 | http://localhost:8051 | Model Context Protocol interface           |
| **Agents Service**         | metis-agents              | http://localhost:8052 | AI/ML operations, reranking                |
| **Agent Work Orders** *(optional)* | metis-agent-work-orders | http://localhost:8053 | Workflow execution with Claude Code CLI    |  

## Upgrading

To upgrade Metis to the latest version:
...
3. **Check for database migrations**:
   - Open the Metis settings in your browser: [http://localhost:3737/settings](http://localhost:3737/settings)
...
By default, Metis services run on the following ports:

- **metis-ui**: 3737
- **metis-server**: 8181
- **metis-mcp**: 8051
- **metis-agents**: 8052 (optional)
- **metis-agent-work-orders**: 8053 (optional)
...
METIS_UI_PORT=3737
METIS_SERVER_PORT=8181
METIS_MCP_PORT=8051
METIS_AGENTS_PORT=8052
...
```bash
METIS_SERVER_PORT=8282
METIS_MCP_PORT=8151
```

### Configuring Hostname

By default, Metis uses `localhost` as the hostname. You can configure a custom hostname or IP address by setting the `HOST` variable in your `.env` file:
...
HOST=metis.local      # Use custom domain
HOST=myserver.com      # Use public domain
```

This is useful when:

- Running Metis on a different machine and accessing it remotely
...
2. Access the UI at: `http://${HOST}:${METIS_UI_PORT}`
...
docker compose logs -f metis-server # API server
docker compose logs -f metis-mcp    # MCP server
docker compose logs -f metis-ui     # Frontend
```
...
- For custom ports, ensure both `METIS_SERVER_PORT` and `VITE_METIS_SERVER_PORT` are set
...
Metis Community License (ACL) v1.2 - see [LICENSE](LICENSE) file for details.

**TL;DR**: Metis is free, open, and hackable. Run it, fork it, share it - just don't sell it as-a-service without permission.
