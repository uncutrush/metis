...
/**
 * Note: For authenticated API calls, components should use the useAuthenticatedAPI hook
 * which automatically includes Clerk JWT tokens in the Authorization header.
 * 
 * This base function accepts headers with Authorization if passed manually,
 * but the recommended approach is to use useAuthenticatedAPI hook.
 */
 ...
 * 
 * For Next.js: Uses relative URLs (proxied through rewrites)
 * For Vite: Uses API_BASE_URL from config
 */
function buildFullUrl(cleanEndpoint: string): string {
  // Check if we're in Next.js environment
  const isNextJS = typeof window !== "undefined" && typeof process !== "undefined" && 
    (process.env?.NEXTJS_ENABLED === "true" || process.env?.NEXT_PUBLIC_API_URL !== undefined);
  
  // In Next.js, use relative URLs (will be proxied through rewrites)
  if (isNextJS) {
    // Ensure endpoint starts with /api
    const cleanPath = cleanEndpoint.startsWith("/") ? cleanEndpoint : `/${cleanEndpoint}`;
    return cleanPath.startsWith("/api") ? cleanPath : `/api${cleanPath}`;
  }

  // For Vite or test environments, use API_BASE_URL
...
export async function callAPIWithETag<T = unknown>(endpoint: string, options: RequestInit = {}): Promise<T> {
  try {
    ...
    // Note: Clerk auth tokens should be added via useAuthenticatedAPI hook
    // or passed manually in options.headers['Authorization']
    // This preserves backward compatibility for unauthenticated requests
