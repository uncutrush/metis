...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
  ...
  const internalHost = 'metis-server';  // Docker service name for internal communication
  ...
  return {
      ...
      proxy: (() => {
        ...
        if (agentWorkOrdersEnabled) {
          proxyConfig['/api/agent-work-orders'] = {
            target: isDocker ? `http://metis-agent-work-orders:${agentWorkOrdersPort}` : `http://localhost:${agentWorkOrdersPort}`,
          changeOrigin: true,
          secure: false,
            timeout: 10000, // 10 second timeout
            configure: (proxy: any, options: any) => {
              const targetUrl = isDocker ? `http://metis-agent-work-orders:${agentWorkOrdersPort}` : `http://localhost:${agentWorkOrdersPort}`;
    ...
    define: {
      ...
      // The browser can't resolve 'metis-server'
