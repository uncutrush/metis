---
description: Execute a development plan with full Metis task management integration
argument-hint: [plan-file-path]
---

# Execute Development Plan with Metis Task Management

You are about to execute a comprehensive development plan with integrated Metis task management. This workflow ensures systematic task tracking and implementation throughout the entire development process.

## Critical Requirements

**MANDATORY**: Throughout the ENTIRE execution of this plan, you MUST maintain continuous usage of Metis for task management. DO NOT drop or skip Metis integration at any point. Every task from the plan must be tracked in Metis from creation to completion.
...
**IMPORTANT**: Create ALL tasks in Metis upfront before starting implementation. This ensures complete visibility of the work scope.
...
### 5.1 Start Task
- Move the current task to "doing" status in Metis: `mcp__metis__manage_task("update", task_id=..., status="doing")`
...
### 5.3 Complete Task
- Once implementation is complete, move task to "review" status: `mcp__metis__manage_task("update", task_id=..., status="review")`
...
## Step 7: Finalize Tasks in Metis

After successful validation:

1. For each task that has corresponding unit test coverage:
   - Move from "review" to "done" status: `mcp__metis__manage_task("update", task_id=..., status="done")`
...
1. **NEVER** skip Metis task management at any point
2. **ALWAYS** create all tasks in Metis before starting implementation
3. **MAINTAIN** one task in "doing" status at a time
4. **VALIDATE** all work before marking tasks as "done"
5. **TRACK** progress continuously through Metis status updates
6. **ANALYZE** the codebase thoroughly before implementation
7. **TEST** everything before final completion

## Error Handling

If at any point Metis operations fail:
1. Retry the operation
2. If persistent failures, document the issue but continue tracking locally
3. Never abandon the Metis integration - find workarounds if needed

Remember: The success of this execution depends on maintaining systematic task management through Metis throughout the entire process. This ensures accountability, progress tracking, and quality delivery.