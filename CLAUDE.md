...
- When commenting on code in the codebase, only comment on the functionality and reasoning behind the code. Refrain from speaking to Metis being in "beta" or referencing anything else that comes from these global rules.

## Development Commands

### Frontend (metis-ui-main/)
...
docker compose logs -f metis-server    # View server logs
docker compose logs -f metis-mcp       # View MCP server logs
docker compose logs -f metis-agent-work-orders  # View agent work orders service logs
docker compose restart metis-server    # Restart after code changes
...
make dev                 # Or manually: docker compose --profile backend up -d && cd metis-ui-main && npm run dev
...
# Terminal 3: cd metis-ui-main && npm run dev
...
- `tasks` - Task tracking linked to projects
  ...
  - Assignee: User, Metis, AI IDE Agent
...
- `GITHUB_REPO_OWNER` - GitHub repository owner (default: "uncutrush")
- `GITHUB_REPO_NAME` - GitHub repository name (default: "Metis")
...
4. Update frontend service in `metis-ui-main/src/features/[feature]/services/`
...
2. View MCP logs: `docker compose logs metis-mcp`
...
- `metis:rag_search_knowledge_base` - Search knowledge base for relevant content
- `metis:rag_search_code_examples` - Find code snippets in the knowledge base
- `metis:rag_get_available_sources` - List available knowledge sources
- `metis:rag_list_pages_for_source` - List all pages for a given source (browse documentation structure)
- `metis:rag_read_full_page` - Retrieve full page content by page_id or URL

### Project Management

- `metis:find_projects` - Find all projects, search, or get specific project (by project_id)
- `metis:manage_project` - Manage projects with actions: "create", "update", "delete"

### Task Management

- `metis:find_tasks` - Find tasks with search, filters, or get specific task (by task_id)
- `metis:manage_task` - Manage tasks with actions: "create", "update", "delete"

### Document Management

- `metis:find_documents` - Find documents, search, or get specific document (by document_id)
- `metis:manage_document` - Manage documents with actions: "create", "update", "delete"

### Version Control

- `metis:find_versions` - Find version history or get specific version
- `metis:manage_version` - Manage versions with actions: "create", "restore"
