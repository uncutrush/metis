# Metis Makefile - Simple, Secure, Cross-Platform
SHELL := /bin/bash
.SHELLFLAGS := -ec
...
help:
	@echo "Metis Development Commands"
	@echo "==========================="
...
dev: check
	...
	echo "Backend running at http://$${HOST:-localhost}:$${METIS_SERVER_PORT:-8181}"
	@echo "Starting frontend..."
	@cd archon-ui-main && \
	VITE_METIS_SERVER_PORT=$${METIS_SERVER_PORT:-8181} \
	VITE_METIS_SERVER_HOST=$${HOST:-} \
	npm run dev
...
dev-docker-full: check
	...
	@$(COMPOSE) up metis-server metis-mcp metis-frontend metis-agent-work-orders -d --build
	@set -a; [ -f .env ] && . ./.env; set +a; \
	echo "✓ All services running"; \
	echo "Frontend: http://localhost:3737"; \
	echo "API: http://$${HOST:-localhost}:$${METIS_SERVER_PORT:-8181}"; \
	echo "MCP: http://$${HOST:-localhost}:$${METIS_MCP_PORT:-8051}"; \
...
agent-work-orders:
	...
	export METIS_SERVER_URL=http://localhost:$${METIS_SERVER_PORT:-8181}; \
	export METIS_MCP_URL=http://localhost:$${METIS_MCP_PORT:-8051}; \
...
dev-work-orders: check
	...
	@$(COMPOSE) up metis-server metis-mcp -d --build
	@set -a; [ -f .env ] && . ./.env; set +a; \
	echo "Backend running at http://$${HOST:-localhost}:$${METIS_SERVER_PORT:-8181}"; \
...
dev-hybrid-work-orders: check
	...
	echo "  Server: http://$${HOST:-localhost}:$${METIS_SERVER_PORT:-8181}"; \
	echo "  MCP: http://$${HOST:-localhost}:$${METIS_MCP_PORT:-8051}"; \
	echo ""; \
	echo "Next steps:"; \
