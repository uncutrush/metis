"""
MCP Server for Metis (Microservices Version)
...
async def perform_health_checks(context: MetisContext):
...
@asynccontextmanager
async def lifespan(server: FastMCP) -> AsyncIterator[MetisContext]:
    """
    ...
    with _initialization_lock:
        ...
        try:
            ...
            # Create context
            context = MetisContext(service_client=service_client)
...
MCP_INSTRUCTIONS = """
# Metis MCP Server Instructions

## 🚨 CRITICAL RULES (ALWAYS FOLLOW)
1. **Task Management**: ALWAYS use Metis MCP tools for task management.
...
try:
    ...
    logger.info("   Server Name: metis-mcp-server")
    logger.info("   Description: MCP server using HTTP calls")

    mcp = FastMCP(
        "metis-mcp-server",
        description="MCP server for Metis - uses HTTP calls to other services",
