...
This service runs independently from the main Metis server and can be deployed:
...
- **Metis Server** (`http://metis-server:8181` or `http://localhost:8181`)
- **Metis MCP** (`http://metis-mcp:8051` or `http://localhost:8051`)
...
1. **Reuse existing Metis Supabase credentials** - No new database or credentials needed. The agent work orders service shares the same Supabase project as the main Metis server.
...
- Ensure same credentials as main Metis server are set
...
docker compose logs -f metis-agent-work-orders
```
...
```bash
# Verify network
docker network inspect metis_app-network

# Test connectivity
docker exec metis-agent-work-orders ping metis-server
docker exec metis-agent-work-orders curl http://metis-server:8181/health
```
