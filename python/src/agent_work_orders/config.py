...
class AgentWorkOrdersConfig:
    ...
    @classmethod
    def get_metis_server_url(cls) -> str:
        """Get Metis server URL based on service discovery mode"""
        # Allow explicit override
        explicit_url = os.getenv("METIS_SERVER_URL")
        if explicit_url:
            return explicit_url

        # Otherwise use service discovery mode
        if cls.SERVICE_DISCOVERY_MODE == "docker_compose":
            return "http://metis-server:8181"
        return "http://localhost:8181"

    @classmethod
    def get_metis_mcp_url(cls) -> str:
        """Get Metis MCP server URL based on service discovery mode"""
        # Allow explicit override
        explicit_url = os.getenv("METIS_MCP_URL")
        if explicit_url:
            return explicit_url

        # Otherwise use service discovery mode
        if cls.SERVICE_DISCOVERY_MODE == "docker_compose":
            return "http://metis-mcp:8051"
