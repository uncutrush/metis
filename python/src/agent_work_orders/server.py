...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    # Log service URLs
    metis_server_url = os.getenv("METIS_SERVER_URL")
    metis_mcp_url = os.getenv("METIS_MCP_URL")

    if metis_server_url:
        logger.info(
            "Service discovery configured",
            extra={
                "metis_server_url": metis_server_url,
                "metis_mcp_url": metis_mcp_url,
            },
        )
...
async def health_check() -> dict[str, Any]:
    ...
    # Check Metis server connectivity (if configured)
    metis_server_url = os.getenv("METIS_SERVER_URL")
    if metis_server_url:
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                response = await client.get(f"{metis_server_url}/health")
                health_status["dependencies"]["metis_server"] = {
                    "available": response.status_code == 200,
                    "url": metis_server_url,
                }
        except Exception as e:
            health_status["dependencies"]["metis_server"] = {
                "available": False,
    ...
    if metis_mcp_url:
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                response = await client.get(f"{metis_mcp_url}/health")
                health_status["dependencies"]["metis_mcp"] = {
                    "available": response.status_code == 200,
                    "url": metis_mcp_url,
                }
        except Exception as e:
    ...
    if supabase_url:
        try:
            ...
            # Check if metis_configured_repositories table exists
            response = client.table("metis_configured_repositories").select("id").limit(1).execute()
