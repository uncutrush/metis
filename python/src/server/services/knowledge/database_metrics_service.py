...
class DatabaseMetricsService:
    ...
    async def get_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            # Sources count
            sources_result = (
                self.supabase.table("metis_sources").select("*", count="exact").execute()
            )
            metrics["sources_count"] = sources_result.count if sources_result.count else 0

            # Crawled pages count
            pages_result = (
                self.supabase.table("metis_crawled_pages").select("*", count="exact").execute()
            )
            ...
            try:
                code_examples_result = (
                    self.supabase.table("metis_code_examples").select("*", count="exact").execute()
                )
    ...
    async def get_storage_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            knowledge_types_result = (
                self.supabase.table("metis_sources").select("metadata->knowledge_type").execute()
            )
