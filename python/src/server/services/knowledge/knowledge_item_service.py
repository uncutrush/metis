...
class KnowledgeItemService:
    ...
    async def list_items(
        ...
        try:
            ...
            query = self.supabase.from_("metis_sources").select("*")
            ...
            count_query = self.supabase.from_("metis_sources").select(
            ...
            if source_ids:
                ...
                urls_result = (
                    self.supabase.from_("metis_crawled_pages")
                ...
                for source_id in source_ids:
                    count_result = (
                        self.supabase.from_("metis_code_examples")
    ...
    async def get_item(self, source_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            result = (
                self.supabase.from_("metis_sources")
                .select("*")
                .eq("source_id", source_id)
                .single()
                .execute()
    ...
    async def update_item(
        ...
        try:
            ...
            if metadata_updates:
                ...
                current_response = (
                    self.supabase.table("metis_sources")
                    .select("metadata")
                    .eq("source_id", source_id)
                    .execute()
                )
    ...
    async def get_available_sources(self) -> dict[str, Any]:
        ...
        try:
            ...
            result = self.supabase.from_("metis_sources").select("*").order("source_id").execute()
    ...
    async def _get_chunks_count(self, source_id: str) -> int:
        ...
        try:
            ...
            result = (
                self.supabase.table("metis_crawled_pages")
                .select("*", count="exact")
                .eq("source_id", source_id)
                .execute()
            )
