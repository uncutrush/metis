...
from ..config.version import METIS_VERSION
...
class MigrationService:
    ...
    async def check_migrations_table_exists(self) -> bool:
        """
        Check if the metis_migrations table exists in the database.
        ...
        try:
                ...
                "sql",
                {
                    "query": """
                        SELECT EXISTS (
                            ...
                            AND table_name = 'metis_migrations'
                        ) as exists
                    """
                }
            ).execute()
            ...
            try:
                ...
                supabase.table("metis_migrations").select("id").limit(0).execute()
                return True
    ...
    async def get_applied_migrations(self) -> list[MigrationRecord]:
        ...
        try:
            ...
            supabase = self._get_supabase_client()
            result = supabase.table("metis_migrations").select("*").order("applied_at", desc=True).execute()
