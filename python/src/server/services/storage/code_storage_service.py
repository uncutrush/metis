...
async def add_code_examples_to_supabase(
    ...
    for url in unique_urls:
        try:
            client.table("metis_code_examples").delete().eq("url", url).execute()
        except Exception as e:
    ...
    for i in range(0, total_items, batch_size):
        ...
        for retry in range(max_retries):
            try:
                client.table("metis_code_examples").insert(batch_data).execute()
                ...
                if retry < max_retries - 1:
                    ...
                    for record in batch_data:
                        try:
                            client.table("metis_code_examples").insert(record).execute()
