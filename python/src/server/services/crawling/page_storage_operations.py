...
Handles the storage of complete documentation pages in the metis_page_metadata table.
...
class PageStorageOperations:
    ...
    Pages are stored in the metis_page_metadata table with full content and metadata.
    ...
    async def store_pages(
        ...
        """
        Store pages in metis_page_metadata table from regular crawl results.
        ...
        if pages_to_insert:
            try:
                safe_logfire_info(
                    f"Upserting {len(pages_to_insert)} pages into metis_page_metadata table"
                )
                result = (
                    self.supabase_client.table("metis_page_metadata")
                ...
                safe_logfire_info(
                    f"Successfully stored {len(url_to_page_id)}/{len(pages_to_insert)} pages in metis_page_metadata"
                )
    ...
    async def store_llms_full_sections(
        ...
        if pages_to_insert:
            try:
                safe_logfire_info(
                    f"Upserting {len(pages_to_insert)} section pages into metis_page_metadata"
                )
                result = (
                    self.supabase_client.table("metis_page_metadata")
                    .upsert(pages_to_insert, on_conflict="url")
    ...
    async def update_page_chunk_count(self, page_id: str, chunk_count: int) -> None:
        ...
        """
        try:
            self.supabase_client.table("metis_page_metadata").update(
