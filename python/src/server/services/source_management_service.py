...
async def update_source_info(
    ...
    try:
        ...
        existing_source = (
            client.table("metis_sources").select("title").eq("source_id", source_id).execute()
        )
        ...
        if existing_source.data:
            ...
            client.table("metis_sources").upsert(upsert_data).execute()
...
class SourceManagementService:
    ...
    def get_available_sources(self) -> tuple[bool, dict[str, Any]]:
        ...
        """
        try:
            response = self.supabase_client.table("metis_sources").select("*").execute()
    ...
    def delete_source(self, source_id: str) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            source_response = (
                self.supabase_client.table("metis_sources")
                .delete()
    ...
    def get_source_details(self, source_id: str) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            # Get page count
            pages_response = (
                self.supabase_client.table("metis_crawled_pages")
                .select("id")
                .eq("source_id", source_id)
                .execute()
            )
            ...
            code_response = (
                self.supabase_client.table("metis_code_examples")
                .select("id")
                .eq("source_id", source_id)
                .execute()
            )
    ...
    def list_sources_by_type(self, knowledge_type: str = None) -> tuple[bool, dict[str, Any]]:
        ...
        """
        try:
            query = self.supabase_client.table("metis_sources").select("*")
