...
def get_container_status_docker() -> dict[str, Any]:
    ...
    try:
        ...
        container = docker_client.containers.get("metis-mcp")
        ...
        return {
            ...
            "uptime": None,
            "logs": [],
            "message": "MCP container not found. Run: docker compose up -d metis-mcp",
        }
...
async def get_mcp_config():
    ...
    with safe_span("api_get_mcp_config") as span:
        ...
        try:
            ...
            mcp_port = int(os.getenv("METIS_MCP_PORT", "8051"))

            # Configuration for streamable-http mode with actual port
            config = {
                "host": os.getenv("METIS_HOST", "localhost"),
                "port": mcp_port,
