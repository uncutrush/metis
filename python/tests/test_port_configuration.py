...
class TestPortConfiguration:
    ...
    def test_service_discovery_requires_all_ports(self):
        ...
        for key in ["METIS_SERVER_PORT", "METIS_MCP_PORT", "METIS_AGENTS_PORT"]:
            os.environ.pop(key, None)

        # Import should fail without environment variables
        with pytest.raises(ValueError, match="METIS_SERVER_PORT environment variable is required"):
    ...
    def test_service_discovery_requires_mcp_port(self):
        ...
        os.environ["METIS_SERVER_PORT"] = "8181"
        os.environ.pop("METIS_MCP_PORT", None)
        os.environ["METIS_AGENTS_PORT"] = "8052"

        with pytest.raises(ValueError, match="METIS_MCP_PORT environment variable is required"):
            from src.server.config.service_discovery import ServiceDiscovery
    ...
    def test_service_discovery_requires_agents_port(self):
        ...
        os.environ["METIS_SERVER_PORT"] = "8181"
        os.environ["METIS_MCP_PORT"] = "8051"
        os.environ.pop("METIS_AGENTS_PORT", None)

        with pytest.raises(ValueError, match="METIS_AGENTS_PORT environment variable is required"):
            from src.server.config.service_discovery import ServiceDiscovery
    ...
    def test_service_discovery_with_all_ports(self):
        ...
        os.environ["METIS_SERVER_PORT"] = "9191"
        os.environ["METIS_MCP_PORT"] = "9051"
        os.environ["METIS_AGENTS_PORT"] = "9052"
    ...
    def test_mcp_server_requires_port(self):
        """Test that MCP server requires METIS_MCP_PORT."""
        os.environ.pop("METIS_MCP_PORT", None)

        # We can't directly import mcp_server.py as it will raise at module level
        # So we test the specific logic
        with pytest.raises(ValueError, match="METIS_MCP_PORT environment variable is required"):
            mcp_port = os.getenv("METIS_MCP_PORT")
            if not mcp_port:
                raise ValueError(
                    "METIS_MCP_PORT environment variable is required. "
                    "Please set it in your .env file or environment. "
                    "Default value: 8051"
                )

    def test_main_server_requires_port(self):
        """Test that main server requires METIS_SERVER_PORT when run directly."""
        os.environ.pop("METIS_SERVER_PORT", None)

        # Test the logic that would be in main.py
        with pytest.raises(ValueError, match="METIS_SERVER_PORT environment variable is required"):
            server_port = os.getenv("METIS_SERVER_PORT")
            if not server_port:
                raise ValueError(
                    "METIS_SERVER_PORT environment variable is required. "
                    "Please set it in your .env file or environment. "
                    "Default value: 8181"
                )

    def test_agents_server_requires_port(self):
        """Test that agents server requires METIS_AGENTS_PORT."""
        os.environ.pop("METIS_AGENTS_PORT", None)

        # Test the logic that would be in agents/server.py
        with pytest.raises(ValueError, match="METIS_AGENTS_PORT environment variable is required"):
            agents_port = os.getenv("METIS_AGENTS_PORT")
            if not agents_port:
                raise ValueError(
                    "METIS_AGENTS_PORT environment variable is required. "
                    "Please set it in your .env file or environment. "
                    "Default value: 8052"
                )

    def test_agent_chat_api_requires_agents_port(self):
        """Test that agent_chat_api requires METIS_AGENTS_PORT for service calls."""
        os.environ.pop("METIS_AGENTS_PORT", None)
    ...
    def test_config_requires_port_or_metis_mcp_port(self):
        """Test that config.py requires PORT or METIS_MCP_PORT."""
        ...
        with pytest.raises(
            ConfigurationError, match="PORT or METIS_MCP_PORT environment variable is required"
        ):
            port_str = os.getenv("PORT")
            if not port_str:
                port_str = os.getenv("METIS_MCP_PORT")
                if not port_str:
                    raise ConfigurationError(
                        "PORT or METIS_MCP_PORT environment variable is required. "
                        "Please set it in your .env file or environment. "
                        "Default value: 8051"
                    )
    ...
    def test_custom_port_values(self):
        ...
        # Set custom ports
        os.environ["METIS_SERVER_PORT"] = "9999"
        os.environ["METIS_MCP_PORT"] = "8888"
        os.environ["METIS_AGENTS_PORT"] = "7777"
...
class TestPortValidation:
    ...
    def test_invalid_port_values(self):
        ...
        os.environ["METIS_SERVER_PORT"] = "not-a-number"
        os.environ["METIS_MCP_PORT"] = "8051"
        os.environ["METIS_AGENTS_PORT"] = "8052"
    ...
    def test_port_out_of_range(self):
        ...
        for port_value, should_succeed in test_cases:
            os.environ["METIS_SERVER_PORT"] = port_value
            os.environ["METIS_MCP_PORT"] = "8051"
            os.environ["METIS_AGENTS_PORT"] = "8052"
