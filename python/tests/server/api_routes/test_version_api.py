...
def mock_version_data():
    ...
    return {
        "current": METIS_VERSION,
        "latest": "0.2.0",
        "update_available": True,
        "release_url": "https://github.com/uncutrush/mehtis/releases/tag/v0.2.0",
        "release_notes": "New features and bug fixes",
        "published_at": datetime(2025, 1, 1, 0, 0, 0),
        "check_error": None,
        "author": "uncutrush",
        "assets": [{"name": "metis.zip", "size": 1024000}],
    }
...
def test_check_for_updates_success(client, mock_version_data):
    ...
    with patch("src.server.api_routes.version_api.version_service") as mock_service:
        ...
        data = response.json()
        assert data["current"] == METIS_VERSION
...
def test_check_for_updates_no_update(client):
    ...
    with patch("src.server.api_routes.version_api.version_service") as mock_service:
        ...
        data = response.json()
        assert data["current"] == METIS_VERSION
        assert data["latest"] == METIS_VERSION
...
def test_get_current_version(client):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["version"] == METIS_VERSION
