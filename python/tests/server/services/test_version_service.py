...
from src.server.config.version import METIS_VERSION
...
def mock_release_data():
    ...
    return {
        "tag_name": "v0.2.0",
        "name": "Metis v0.2.0",
        "html_url": "https://github.com/uncutrush/mehtis/releases/tag/v0.2.0",
        "body": "## Release Notes\n\nNew features and bug fixes",
        "published_at": "2025-01-01T00:00:00Z",
        "author": {"login": "uncutrush"},
        "assets": [
            {
                "name": "metis-v0.2.0.zip",
                "size": 1024000,
                "download_count": 100,
                "browser_download_url": "https://github.com/uncutrush/mehtis/releases/download/v0.2.0/archon-v0.2.0.zip",
...
async def test_check_for_updates_new_version_available(version_service, mock_release_data):
    ...
    with patch.object(version_service, "get_latest_release", return_value=mock_release_data):
        ...
        assert result["current"] == METIS_VERSION
        ...
        assert result["author"] == "uncutrush"
...
async def test_check_for_updates_same_version(version_service):
    ...
    mock_data = {"tag_name": f"v{METIS_VERSION}", "html_url": "test_url", "body": "notes"}

    with patch.object(version_service, "get_latest_release", return_value=mock_data):
        result = await version_service.check_for_updates()

        assert result["current"] == METIS_VERSION
        assert result["latest"] == METIS_VERSION
