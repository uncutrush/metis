...
def test_config_local_service_discovery():
    ...
    assert config.SERVICE_DISCOVERY_MODE == "local"
    assert config.get_metis_server_url() == "http://localhost:8181"
    assert config.get_metis_mcp_url() == "http://localhost:8051"
...
def test_config_docker_service_discovery():
    ...
    assert config.get_metis_server_url() == "http://metis-server:8181"
    assert config.get_metis_mcp_url() == "http://metis-mcp:8051"


@pytest.mark.unit
@patch.dict("os.environ", {"METIS_SERVER_URL": "http://custom-server:9999"})
def test_config_explicit_server_url_override():
    """Test explicit METIS_SERVER_URL overrides service discovery"""
    from src.agent_work_orders.config import AgentWorkOrdersConfig

    config = AgentWorkOrdersConfig()

    assert config.get_metis_server_url() == "http://custom-server:9999"


@pytest.mark.unit
@patch.dict("os.environ", {"METIS_MCP_URL": "http://custom-mcp:7777"})
def test_config_explicit_mcp_url_override():
    """Test explicit METIS_MCP_URL overrides service discovery"""
    from src.agent_work_orders.config import AgentWorkOrdersConfig

    config = AgentWorkOrdersConfig()

    assert config.get_metis_mcp_url() == "http://custom-mcp:7777"
...
@patch.dict(
    "os.environ",
    {
        ...
        "METIS_SERVER_URL": "http://explicit-server:8888",
    },
)
def test_config_explicit_url_overrides_discovery_mode():
    ...
    assert config.SERVICE_DISCOVERY_MODE == "docker_compose"
    assert config.get_metis_server_url() == "http://explicit-server:8888"
