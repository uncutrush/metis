...
@patch.dict("os.environ", {"METIS_SERVER_URL": "http://localhost:8181", "ENABLE_AGENT_WORK_ORDERS": "true"})
async def test_health_check_server_connectivity(mock_client_class):
    ...
    assert result["dependencies"]["metis_server"]["available"] is True
    assert result["dependencies"]["metis_server"]["url"] == "http://localhost:8181"


@pytest.mark.unit
@patch("src.agent_work_orders.server.httpx.AsyncClient")
@patch.dict("os.environ", {"METIS_MCP_URL": "http://localhost:8051", "ENABLE_AGENT_WORK_ORDERS": "true"})
async def test_health_check_mcp_connectivity(mock_client_class):
    ...
    assert result["dependencies"]["metis_mcp"]["available"] is True
    assert result["dependencies"]["metis_mcp"]["url"] == "http://localhost:8051"
...
async def test_health_check_server_unavailable(mock_client_class):
    ...
    assert result["dependencies"]["metis_server"]["available"] is False
    assert "error" in result["dependencies"]["metis_server"]
