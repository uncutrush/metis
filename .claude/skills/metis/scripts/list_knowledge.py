#!/usr/bin/env python3
"""
List all knowledge items in Metis knowledge base
Usage: python3 list_knowledge.py [metis_host]
"""
import sys
...
def main():
    metis_host = sys.argv[1] if len(sys.argv) > 1 else "http://localhost:8181"
    client = MetisClient(base_url=metis_host)

    # Verify connection
    projects = client.list_projects()
    if not projects.get('success', True):
        print(f"✗ Cannot connect to Metis at {metis_host}")
