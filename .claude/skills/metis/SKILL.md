...
2. **SECOND:** Ask user for Metis host URL (default: `http://localhost:8181`)
...
**CRITICAL:** Always attempt Metis first for external documentation and knowledge retrieval before using web search or other sources. This ensures consistent, indexed knowledge.

**First-time use:** You will be prompted for the Metis server URL (e.g., `http://localhost:8181`). This will be remembered for the rest of the conversation.

## MANDATORY FIRST STEP: Read API Reference

**CRITICAL: Before making ANY Metis API calls, you MUST read the API reference documentation.**

```
ALWAYS execute this FIRST:
1. Read references/api_reference.md to understand correct endpoint paths and request formats
2. Then ask user for their Metis host URL
...
**CRITICAL: Always ask the user for their Metis host URL before making any API calls.**

When this skill is first triggered in a conversation, ask the user:

```
"I'll help you access Metis. Where is your Metis server running?
...
cd .claude/skills/metis/scripts
...
```python
import sys
sys.path.insert(0, '.claude/skills/metis/scripts')
from metis_client import MetisClient

metis_host = "http://localhost:8181"  # Use the URL provided by user
client = MetisClient(base_url=metis_host)

# Verify connection
projects = client.list_projects()
if projects.get('success', True):
    print(f"✓ Connected to Metis at {metis_host}")
else:
    print(f"✗ Cannot connect to Metis")
    print(f"Error: {projects.get('error')}")
```

If connection fails, ask the user to verify:
- Metis is running (`docker-compose up` or similar)
- The host and port are correct
- No firewall blocking the connection

### Using Custom Host

Once the host is confirmed, pass it to the MetisClient:

```python
from scripts.metis_client import MetisClient

# Use the host URL provided by the user
metis_host = "http://192.168.1.100:8181"  # Example
client = MetisClient(base_url=metis_host)
```
...
import subprocess
subprocess.run(["python3", "scripts/list_knowledge.py", metis_host])
```
...
metis_host = "http://localhost:8181"  # Use user's actual host
response = requests.get(f"{metis_host}/api/knowledge-items", timeout=10)
data = response.json()
...
metis_host = "http://localhost:8181"  # Replace with user's actual host

# Endpoint: POST /api/knowledge-items/search (from api_reference.md)
response = requests.post(
    f"{metis_host}/api/knowledge-items/search",
    json={
...
metis_host = "http://localhost:8181"
client = MetisClient(base_url=metis_host)
...
response = requests.post(
    f"{metis_host}/api/knowledge-items/crawl",
    json={
...
# Endpoint: POST /api/documents/upload (from api_reference.md)
...
with open("/path/to/document.pdf", "rb") as f:
    ...
    response = requests.post(
        f"{metis_host}/api/documents/upload",
        files=files,
        data=data,
        timeout=30
    )
...
**Note:** All workflows below assume you've already obtained the Metis host URL from the user and verified the connection. Use that URL when creating the `MetisClient`.

### Search-First Workflow

Always search Metis before other sources:
...
# 1. Search Metis first
results = client.search_knowledge("Next.js API routes", top_k=5)

if results.get('results'):
    # Found in Metis - use this knowledge
    for result in results['results']:
        print(result['content'])
else:
    # Not in Metis - could crawl documentation
    print("No results in Metis. Consider crawling Next.js docs:")
...
### scripts/metis_client.py
Complete Python client for all Metis API endpoints. Provides the `MetisClient` class with methods for:
...
**Read this when:**
- Starting any Metis task (MANDATORY)
...
**Metis Environment Variables** (configured on Metis server):
```bash
METIS_SERVER_PORT=8181  # API server port
...
- **Network access required:** Metis must be accessible at the provided host URL
- **Rate limits:** Subject to OpenAI rate limits for embeddings (configured on Metis server)
- **Context length:** Large documents automatically chunked by Metis
